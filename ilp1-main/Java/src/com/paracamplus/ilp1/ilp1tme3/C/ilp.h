...
typedef struct ILP_Object {
     ...
     union {
          ...
          struct asVector {
               int      _size;
               struct ILP_Object*     asVectorObjet[1];
          } asVector;
     }                  _content;
} *ILP_Object;
...
#define ILP_AllocateVector(length) ILP_malloc(sizeof(struct ILP_Object) + (sizeof(struct ILP_Object)*length), &ILP_object_Vector_class)
