...
/* Fonctions ajoutees par nous */
/* Sinus */
ILP_Object
ILP_sinus (ILP_Object self){
     if (ILP_isInteger(self))
     	return ILP_make_float(sin(self->_content.asInteger));
     else if (ILP_isFloat(self))
     	return ILP_make_float(sin(self->_content.asFloat));
     	
     return ILP_domain_error("Error", self);
}
/* Classe Vector */
...
ILP_makeVector (ILP_Object taille, ILP_Object valeur){
     if (ILP_isInteger(taille)){
          int tailleBis = taille->_content.asInteger;
          if (tailleBis >= 0){
               ILP_Object vecteur = ILP_AllocateVector(tailleBis);
               vecteur->_content.asVector._size = tailleBis;
               for (int i=0; i<tailleBis; ++i)
                    vecteur->_content.asVector.asVectorObjet[i] = valeur;
               
               return vecteur;
          } else ILP_domain_error("La taille doit etre >= 0", taille);
     } else ILP_domain_error("La taille n'est pas un int", taille); 
     
     return ILP_domain_error("Error", taille);
}
/* VectorLength */
ILP_Object 
ILP_vectorLength (ILP_Object vecteur){
     if (ILP_isVector(vecteur))
          ...
          return ILP_domain_error("Ce n'est pas un vecteur", vecteur); 
}
/* VectorGet */
ILP_Object  
ILP_vectorGet (ILP_Object vecteur, ILP_Object index){
     if (ILP_isVector(vecteur)){
          if (ILP_isInteger(index)){
               int indexBis = index->_content.asInteger;
               //mai mic k
               if (indexBis >= 0){ 
                    return vecteur->_content.asVector.asVectorObjet[indexBis];
               } else return ILP_domain_error("Index negatif", index);
          } else return ILP_domain_error("Ce n'est pas un int", index);

     } else return ILP_domain_error("Ce n'est pas un vectuer", vecteur);
}
