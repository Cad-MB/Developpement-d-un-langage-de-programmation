...
ILP_Object receiverCache;
ILP_Method methodCache;
int argcCache;
ILP_general_function resultCache;
int cptAppel = 0;
int cacheHit = 0;
...
{
	 cptAppel++;
     if ((receiverCache == receiver) && (methodCache == method) && (argcCache == argc)){
 		cacheHit++;	
     	return resultCache;
 	 } else{
	 	 resultCache = ILP_find_method(receiver, method, argc);
		 receiverCache = receiver;
		 methodCache = method;
		 argcCache = argc;
		 return resultCache;
	}	
...
/* Ajout TME9 */
void affichage_cpts(){
	FILE* f = fopen("Java/src/com/paracamplus/ilp4/ilp4tme9/result.txt", "a");
	fprintf(f, "Nombre d'appels de methodes: %d | Nombre de cache hits: %d\n", cptAppel, cacheHit);
	fclose(f);
}
